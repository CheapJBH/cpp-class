# C++ 算法笔记

## C++官网

https://zh.cppreference.com

## 输入输出

```cpp
ios::sync_with_stdio(false);
cin.tie(0), cout.tie(0);
```

```cpp
cout << fixed << setprecision(1) << a;    // 四舍五入保留小数
```

```cpp
// 无限读入，Ctrl+Z 结束
while (cin >> n) { ... }
```

## 数学

```cpp
abs(x);                // 绝对值
int(ceil(a));          // 向上取整
swap(a, b);            // 交换
```

```cpp
// 判断质数：枚举 2 ~ sqrt(n)
```

```cpp
// 向上取整
int(ceil(a));
```

## 字符串

```cpp
string s;
cin.getline(a + 1, n);        // 读入含空格，下标从 1 开始
s.size();                      // 长度
reverse(s.begin(), s.end());
sort(s.begin(), s.end());
s.substr(3, 6);                // 下标 3 ~ 6 的子串
s.substr(1);                   // 下标 1 ~ n
s.find(n);                     // 返回下标，未找到返回 -1
```

## 数据结构

### vector

```cpp
vector<int> vec;
vec.push_back(5);
vec.size();
vec.pop_back();
vec.clear();
for (int i : vec) { cout << i << endl; }
```

### set（自动排序 + 去重）

```cpp
set<int> s;
s.insert(x);             // O(log n)
s.erase(x);              // O(log n)
s.count(x);              // 判断是否存在，O(log n)
s.size();                // O(1)
s.empty();               // O(1)
*s.begin();              // 最小值
for (int i : s) { cout << i; }
```

### map

```cpp
map<int, int> mp;
mp[1] = 2;               // 1 -> 2
mp[1000010000] = 1;      // 不报错
cout << mp[8];           // 默认 0
mp.size();               // 映射关系数
mp.count(9);             // 判断单个映射是否存在
mp.empty();              // 判断是否为空
mp.erase(100);           // 删除
for (auto it : mp) {
    cout << it.first << it.second;
}
```

### priority_queue

```cpp
priority_queue<int> q;                               // 大根堆
priority_queue<int, vector<int>, greater<int>> q;     // 小根堆
q.push(x);
q.top();
q.pop();
q.size();
q.empty();
```

### struct + 重载运算符

```cpp
struct node
{
    int x, y;
    friend bool operator < (node p, node q)
    {
        if (p.x != q.x) return p.x < q.x;
        return p.y < q.y;
    }
};
```

## 算法

### 排序

```cpp
sort(a + 1, a + n + 1);
reverse(a + 1, a + n + 1);
```

### 二分 / 三分

```cpp
// lower_bound 返回第一个 >= x 的迭代器
*lower_bound(vec.begin(), vec.end(), a[i]) = a[i];

// 三分
l = (r - l) / 3;
```

### 全排列

```cpp
sort(a + 1, a + n + 1);
do
{
    // ...
} while (next_permutation(a + 1, a + n + 1));
```

### 二进制枚举

```cpp
for (int i = 1; i < (1 << n); i++)
{
    for (int j = n - 1; j >= 0; j--)
    {
        cout << (i >> j) % 2;
    }
    cout << endl;
}
```

### 前缀和

```cpp
// 一维
p[i] = p[i - 1] + a[i];
// 区间和：p[r] - p[l - 1]
//一维前缀和是离散意义下的积分
//一维差分是离散意义下的求导

// 二维
p[i][j] = p[i-1][j] + p[i][j-1] - p[i-1][j-1] + a[i][j];
// 子矩阵和：p[x2][y2] - p[x1-1][y2] - p[x2][y1-1] + p[x1-1][y1-1]
```

### 差分

```cpp
// 区间加：d[l] += x, d[r + 1] -= x
// 前缀和后得到原数组
```

## 动态规划

### 背包

```cpp
// 0-1 背包
for (int i = 1; i <= n; i++)
{
    for (int j = m; j >= v[i]; j--)
    {
        f[j] = max(f[j], f[j - v[i]] + c[i]);
    }
}

// 可行性背包
f[j] |= f[j - a[i]];
```

### 区间 DP

```cpp
for (int len = 2; len <= n; len++)
{
    for (int i = 1; i + len - 1 <= n; i++)
    {
        int j = i + len - 1;
        f[i][j] = min(f[i][j], f[i][k] + f[k + 1][j] + w(i, j));
    }
}
```

### 最长上升子序列

```cpp
// O(n log n)
for (int i = 1; i <= n; i++)
{
    if (vec.empty() || a[i] > vec.back())
        vec.push_back(a[i]);
    else
        *lower_bound(vec.begin(), vec.end(), a[i]) = a[i];
}
```

### 错排方案

```cpp
f[n] = (n - 1) * (f[n - 2] + f[n - 1]);
```

## 图论

### DFS

```cpp
bool vis[maxn];

void dfs(int u)
{
    if (vis[u]) return;
    vis[u] = true;
    for (int v : vec[u])
    {
        dfs(v);
    }
}
```

### BFS

```cpp
int dis[maxn];
memset(dis, -1, sizeof(dis));
queue<int> q;
q.push(1);
dis[1] = 0;

while (!q.empty())
{
    int u = q.front(); q.pop();
    for (int v : vec[u])
    {
        if (dis[v] != -1) continue;
        dis[v] = dis[u] + 1;
        q.push(v);
    }
}
```

## 近期题目

### A - 序列计数（2026.6.21）

> 用 ≥3 的整数组成 n，求方案数，模 998244353。

```cpp
f[0] = 1, f[1] = 0, f[2] = 0;
for (int i = 3; i <= n; i++)
{
    f[i] = (f[i - 1] + f[i - 3]) % MOD;
}
```

### B - 多边形（CSP-J 2025）

> 给定 n 根木棍长度，求能组成多边形的子集数，模 998244353。

```cpp
sort(a + 1, a + n + 1);
dp[0] = 1;
int cur = 0;
for (int i = 1; i <= n; i++)
{
    for (int j = cur; j >= 0; j--)
    {
        dp[j + a[i]] = (dp[j + a[i]] + dp[j]) % MOD;
    }
    for (int j = 0; j <= a[i]; j++)
    {
        ans = (ans + dp[j]) % MOD;
    }
    cur += a[i];
}
```

## 开发环境

```cpp
// Dev-C++ 设置
// 工具 -> 编译选项 -> 代码生成/优化 -> 语言标准 -> GNU C++ 11
```

### 代码规范

- `#include<bits/stdc++.h>` + `using namespace std;`
- `#define endl '\n'` 提升性能
- `using lnt = long long;` — **不得**使用 `#define int long long`
- `int main()` — **不得**使用 `signed main()`
- 全部逻辑写在 `void Solve(void)` 内
- `int main()` 包含 ONLINE_JUDGE guard + `ios::sync_with_stdio(false); cin.tie(nullptr);` + `while (T--)Solve();`
- 全局数组，无函数（若有函数则写在 Solve 前）
- 括号换行（函数、if、for、while），例外：`int main()` 大括号不换行
- `/*====================*/` 分隔各个代码段
- `maxn` → `N`（大写字母常量名）
- 输出中文无需额外处理（`-include utf8fix.h`）

### Git 工作流

```bash
git init
git add -A
git commit -m "信息"
# 推送
git remote add origin <url>
git push -u origin main
```

## 文件夹结构

```
C:\Visual Studio Code\
├── C++\                  # C++ 代码（按日期）
│   ├── 2025.12.28/       # BFS
│   ├── 2026.3.8/         # 综合练习
│   ├── 2026.3.8-2/       # DP
│   ├── 2026.3.22/        # DP2
│   ├── 2026.3.22-2/      # DP3
│   ├── 2026.4.12/        # 背包
│   ├── 2026.4.12-2/      # 背包进阶
│   ├── 2026.4.19/        # DP4
│   ├── 2026.4.26/        # DP5
│   ├── 2026.5.10/        # 小游戏 + DP6
│   ├── 2026.5.16/        # 综合练习
│   ├── 2026.5.24/        # 综合练习
│   ├── 2026.5.31/        # 素数筛、并查集、MST
│   ├── 2026.6.7/         # 约数个数、质因数分解
│   ├── 2026.6.21/        # 序列计数、多边形
│   ├── 2026.7.4/         # AtCoder 综合
│   ├── 2026.7.5/         # AtCoder 综合
│   ├── 笔记.md
│   └── README.md
├── Python/
│   ├── game.py
│   └── 爬虫.py
├── README.md
└── .gitignore
```

## 杂项

```cpp
for (;;) ;               // 无限循环
unsigned int;             // 正整数
break;                    // 跳出
continue;                 // 执行下一次
```

```cpp
#define 替换项 替换物
```
